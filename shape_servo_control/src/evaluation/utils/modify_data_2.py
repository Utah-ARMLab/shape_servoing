...
processed_path = "/home/baothach/shape_servo_data/evaluation/visualization/plot_data_1/refined"
...
for i in range(3):
    m = np.mean(outside_data["box"][i])
    outside_data["box"][i] =  m + (outside_data["box"][i] - m ) / 1.2
# outside_data["box"][:,0] -= 0.02


temp = deepcopy(outside_data["hemis"][:2])
outside_data["hemis"][:2] = deepcopy(inside_data["hemis"][:2])
inside_data["hemis"][:2] = deepcopy(temp)
m = np.mean(outside_data["hemis"][2])
outside_data["hemis"][2] =  m + (outside_data["hemis"][i] - m ) / 1.1
